...
async function tiktok2 (url) {
	...
	try {
		...
		urls.slice(0, -4)
		if (urls[0].length < 100) return resolve({ creator: '@Wan – Norman Wawan', status: false })
		let urlList = {
			video: 'https://www.ggtiktok.com' + urls[0],
			videoWM: 'https://www.ggtiktok.com' + urls[1]
		}
		resolve({ creator: '@Wan – Norman Wawan', status: true, data: urlList })
	} catch {
		return resolve({ creator: '@Wan – Norman Wawan', status: false })
	}})
}
